...
### Added
- Added `scripts/check_release_version.sh` plus a tracked `.githooks/pre-commit` hook to enforce that `VERSION` matches `release/X.Y.Z[-rcN]` or `release/X.Y.Z[-rc.N]` branch names during local commits.
